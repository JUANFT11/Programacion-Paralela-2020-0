...
void Read_matrix( //
   ...
   if (my_rank == 0) {
      A = malloc(m*n*sizeof(double)); //array temporal
      ...
      for (i = 0; i < m; i++)
         for (j = 0; j < n; j++)
            scanf("%lf", &A[i*n+j]);
      MPI_Scatter(A, local_m*n, MPI_DOUBLE, //toma el total de procesos, los segmenta y lo redstribuye a todos incluyendose a si mismo
