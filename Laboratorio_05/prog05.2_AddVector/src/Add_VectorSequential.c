...
int main(void) {
   ...
   Read_vector(x, n, "x");  //iNGRESA LOS ELEMENTO MANUALMENTE, lee los elementos para el vecotor X
   Read_vector(y, n, "y"); //iNGRESA LOS ELEMENTO MANUALMENTE, lee los elementos para el vecotor Y

   Vector_sum(x, y, z, n); //precede con la suma de los vectores con parametros dfe entrada y salida

   Print_vector(z, n, "The sum is"); //imprime la suma

   //libera la memoria de x,y, z
   free(x);
   free(y);
   free(z);
